	...
	function act(){
		for(var j=0;j<item;j++){
			if(flag == false){
				...
				if(color_flag == false){
					if((color_16_1_obj[j] <160)&&(color_state[j] == 0)){
		 				color_16_1_obj[j] += 1;				
					}
					else if(color_16_1_obj[j] == 160){
						color_state[j] = 1;
					}
					if((color_16_2_obj[j] <210)&&(color_state[j] == 1)){
		 				color_16_2_obj[j] += 1; 	  						
					}
					else if(color_16_2_obj[j] == 210){
						color_state[j] = 2;
					}
					if((color_16_3_obj[j] <150)&&(color_state[j] == 2)){
		 				color_16_3_obj[j] += 1;  			 						
					}
					...
					if((color_16_1_back_obj[j]+30 <color_16_1_obj[j])&&(color_state[j] == 0)){
		 				color_16_1_obj[j] -= 1;					
					}
					else if(color_16_1_back_obj[j]+30 == color_16_1_obj[j]){
						color_state[j] = 1;
					}
					if((color_16_2_back_obj[j]+20 <color_16_2_obj[j])&&(color_state[j] == 1)){
		 				color_16_2_obj[j] -= 1;					
					}
					else if(color_16_2_back_obj[j]+20 == color_16_2_obj[j]){
						color_state[j] = 2;
					}
					if((color_16_3_back_obj[j]+50 <color_16_3_obj[j])&&(color_state[j] == 2)){
		 				color_16_3_obj[j] -= 1;					
					}
					else if(color_16_3_back_obj[j]+50 == color_16_3_obj[j]){
						color_state[j] = 0;
					}
				}
			...
			for(var i=0;i<=18;i++){
				if(count_time == 3){
					...
					out_control_point_obj[j][i] += (25 + circle_size+40-circle_bold*j -out_control_point_obj[j][i]) / (time_end/2); 
 					...
 					out_point_obj[j][i] -= 1.5*Math.random()*(scale_point-time*(scale_point/time_end));
					out_control_point_obj[j][i] -= 1.5*Math.random()*(scale_control-time*(scale_control/time_end)); 
					if(time == time_end){
						time = 0;
						count_time++;
						flag = true;
